...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if "xch_target_address" not in config["farmer"]:
        print(
            f"Setting the kik destination for the farmer reward (1/8 plus fees, solo and pooling) to {all_targets[0]}"
        )
    ...
    if "xch_target_address" not in config["pool"]:
        print(f"Setting the kik destination address for pool reward (7/8 for solo only) to {all_targets[0]}")
...
def chia_init(
    ...
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if CHIA_ROOT is set, or if user has run kiwi init twice
        # before a new update.
        if testnet:
