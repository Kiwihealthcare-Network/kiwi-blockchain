...
- Migration with CHIA_ROOT set does not crash kiwi init
...
- `kiwi init` only migrates release candidate directories. The versioned sub directories under `~/chia` will be going away before mainnet.
...
- Use 'kiwi wallet get_transactions' in the command line to see your transactions.
- 'kiwi wallet show' now shows your wallet's height.
...
- 'kiwi wallet send' now takes in TXCH or XCH as units instead of mojos.
...
- `kiwi init -c [directory]` will create new TLS certificates signed by your CA located in `[directory]`. Use this feature to configure a new remote harvester. Type `kiwi init -h` to get instructions. Huge thanks to a very efficient @eFishCent for this quick and thorough pull request.
...
- `kiwi init` now automatically discovers previous releases in each new release.
...
- You can now create or import private keys with a 24 word mnemonic, both in the UI and 'kiwi keys' command line.
...
- `kiwi init` properly migrates from previous versions including the k>=32 workaround. Additionally, the farming target key is checked to make sure that it is the valid and correct public key format.
...
- There is a new command `kiwi init` that will migrate files and generate your initial configuration. If you want to use the Wallet or farm, you will also have to `chia-generate-keys`. You can read step by step instructions for [upgrading from a previous beta release](https://github.com/Chia-Network/chia-blockchain/wiki/Updating-beta-software). If you've set `$CHIA_ROOT` you will have to make sure your existing configuration remains compatible manually.
